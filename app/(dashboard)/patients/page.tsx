...
import { Plus, Search, Filter, MoreHorizontal, Mail, Phone, Star, Eye, Edit2, Trash2 } from "lucide-react"
...
export default function PatientsPage() {
  ...
  const filteredPatients = mockPatients.filter((p) => {
    ...
    if (filterVip) return matchesSearch && p.vip
      ...
      <Tabs defaultValue="all">
                    ...
                    {filteredPatients.map((patient, i) => (
                      ...
                      >
                        <td className="px-3 sm:px-6 py-3 sm:py-4">
                          <Link href={`/patients/${patient.id}`} className="flex items-center gap-2 sm:gap-3">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-xs text-slate-400 truncate">{patient.gender === "male" ? "Male" : "Female"} · {patient.dateOfBirth}</p>
                            </div>
                          </Link>
                        </td>
                        ...
                        <td className="px-3 sm:px-6 py-3 sm:py-4 hidden lg:table-cell">
                          <span className="text-sm text-slate-500">{patient.lastVisit || "N/A"}</span>
                        </td>
                        <td className="px-3 sm:px-6 py-3 sm:py-4">
                        ...
                        <td className="px-3 sm:px-6 py-3 sm:py-4 hidden xl:table-cell">
                          <div className="flex flex-wrap gap-1">
                            {patient.tags.map((tag) => (
                              <Badge
                                key={tag}
                                variant={tag === "VIP" ? "primary" : tag === "Blacklisted" ? "destructive" : "default"}
          ...
          <Card>
            <CardContent className="p-6">
              <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                {[...mockPatients].sort((a, b) => new Date(b.lastVisit || "").getTime() - new Date(a.lastVisit || "").getTime()).slice(0, 6).map((patient) => (
                  <Link key={patient.id} href={`/patients/${patient.id}`}>
                    <div className="flex items-center gap-4 p-4 rounded-xl border border-slate-200 dark:border-slate-700 hover:shadow-md hover:border-teal-200 dark:hover:border-teal-900/50 transition-all duration-200">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400">Last visit: {patient.lastVisit || "Never"}</p>
        ...
        <TabsContent value="vip">
          <Card>
            <CardContent className="p-6">
              <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                {mockPatients.filter(p => p.vip).map((patient) => (
